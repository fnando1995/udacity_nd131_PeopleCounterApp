...
class Tracker(object):
    def __init__(self,THRESH,MatrixOPeration,AssigProblemSolver,FPS = 10):
        ...
        self.video_fps              =   FPS
        self.tracked_frames_number  =   [0,0]   # [sum of seconds,number of tracks],
        ...
        if len(erased_tracks)!=0:
            for erased in erased_tracks:
                ...
                if seconds > self.tracks_time_thresh:
                    self.tracked_frames_number[0] += frames_count
                    self.tracked_frames_number[1] += 1
                    average_time = (self.tracked_frames_number[0]/self.video_fps)/self.tracked_frames_number[1]
